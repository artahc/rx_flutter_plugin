...
class RxFlutterMethodChannel {
  ...
  late TestDefaultBinaryMessenger _testDefaultBinaryMessenger;

  RxFlutterMethodChannel(String channelName) {
  ...
  /// Always return null.
  Future<dynamic> _methodChannelCallback(MethodCall call) async {
    ...
    try {
      ...
      _log.w("Failed to cast MethodCall.arguments to an ObservableCallback. Skipping callback processing.",
          e as Exception, stack);
      return null;
    }
    ...
    if (streamController == null) {
      _log.w(
          "Unable to find streamController for requestId ${callback.requestId}. Unable to proceed with processing MethodCall");
      return null;
    }
    ...
    switch (callback.type) {
      ...
      case ObservableCallbackType.onError:
        ...
        streamController.addError(exceptionToThrow);
        streamController.close();
        break;
    }
    return null;
  ...
  Stream<dynamic> getObservable(String method, [dynamic arguments]) {
  ...
  Stream<dynamic> getSingle(String method, [dynamic arguments]) {
    final requestId = Utils.generateRandomInt();
  ...
  Stream<void> getCompletable(String method, [dynamic arguments]) {
    final requestId = Utils.generateRandomInt();
  ...
  ObservableStreamController<dynamic> _createSteamController(
      String method, int requestId, dynamic arguments, StreamType streamType) {
    ObservableStreamController<dynamic> streamController;
    streamController = new ObservableStreamController(streamType, onListen: () async {
      //Send observable registration
      final _ = await _subscribeToObservable(
          ObservableRegistrationRequest(method, streamType, requestId, arguments, ObservableAction.subscribe));
    }, onCancel: () async {
      ...
      final _ = await _subscribeToObservable(
          ObservableRegistrationRequest(method, streamType, requestId, null, ObservableAction.dispose));
    });
